...
type Observation = {
  ...
  error?: string;
  previews: Array<{ maxDimension: number; status: string; durationMs: number; rawText?: string; score?: number; error?: string }>;
};
...
export async function probeLegacyDirection(image: Buffer, factory?: () => Promise<ProbeWorker>) {
  ...
  const preview = await sharp(image).resize({ width: 1024, height: 1024, fit: 'inside', withoutEnlargement: true }).png().toBuffer();
  const meta = await sharp(preview).metadata();
  let finePreview: Buffer | undefined;
  ...
  try {
    for (const rotation of [0, 90, 180, 270]) {
      ...
      const observation: Observation = { rotation, startedAt: new Date().toISOString(), started: true, recognitionStarted: false, status: 'errored', durationMs: 0, score: 0, credible: false, words: 0, unique: 0, lines: 0, rawText: '', signals: { headers: 0, money: 0, dates: 0 }, previews: [] };
      try {
        if (!worker) {
          let expired = false;
        ...
        // Both previews share one deadline. Small readable text needs no larger
        // pass; faint small characters may use only the remaining budget.
        const deadline = performance.now() + CANDIDATE_MS;
        for (const maxDimension of [1024, 1800]) {
          if (maxDimension === 1800 && (observation.credible || Math.max(meta.width!, meta.height!) < 1024)) break;
          const passStart = performance.now();
          try {
            if (maxDimension === 1800 && !finePreview) finePreview = await sharp(image).resize({ width: 1800, height: 1800, fit: 'inside', withoutEnlargement: true }).png().toBuffer();
            const rotated = await sharp(maxDimension === 1024 ? preview : finePreview!).rotate(rotation).png().toBuffer();
            const remaining = deadline - performance.now();
            if (remaining <= 0) throw new ProbeTimeout('Orientation observation exceeded 2 seconds');
            observation.recognitionStarted = true;
            const result = await bounded(worker.recognize(rotated, {}, { text: true, blocks: true }), remaining, 'Orientation observation exceeded 2 seconds');
            const evidence = directionEvidence(result.data);
            observation.previews.push({ maxDimension, status: 'completed', durationMs: performance.now() - passStart, rawText: result.data.text, score: evidence.score });
            if (observation.status !== 'completed' || evidence.score > observation.score) {
            ...
            observation.previews.push({ maxDimension, status: error instanceof ProbeTimeout ? 'timed-out' : 'errored', durationMs: performance.now() - passStart, error: error instanceof Error ? error.message : String(error) });
            throw error;
          }
        }
      } catch (error) {
