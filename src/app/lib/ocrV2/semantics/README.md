...
| Header fit from at least 3 independent semantic columns; perpendicular residual within 0.6 glyph height | Preserve sloped headings across passes; duplicate observations cannot establish skew. Residual slope is limited to 15 degrees, and incompatible competing fits fall back to flat mapping. This changes geometry only, never monetary authority. |
...
Physical rows are frozen from slope-projected word-box geometry, multi-column support and whitespace boundaries before field recognition. Transcribed invoice tokens never create/delete rows. Pixel components complete field crop bounds; projected half-open ownership prevents row bleed. The private corpus/overlay scripts write evidence outside Git. Unknown fields retain their PhysicalRow diagnostics.
