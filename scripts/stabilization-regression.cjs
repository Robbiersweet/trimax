...
async function retryStateRegression() {
  ...
  assert.equal(failureWrite.summary.result,'failed');assert.equal(failureWrite.payload.transport.httpStatus,413);assert.equal(failureWrite.payload.transport.ocrStarted,false);
