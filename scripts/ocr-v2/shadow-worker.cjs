...
const toWSL=file=>{const full=path.resolve(file);if(!/^[A-Za-z]:\\/.test(full))throw Error('Expected Windows path');return '/mnt/'+full[0].toLowerCase()+'/'+full.slice(3).replaceAll('\\','/');};
...
async function once(){
 ...
 try{
  ...
  const result=await runShadowPipeline(original,{attemptId:id,captureSessionId:job.capture_session_id,sourceImageHash:job.source_hash,build:cp.execFileSync('git',['rev-parse','HEAD'],{cwd:repository,encoding:'utf8'}).trim(),snapshot:claim.input.snapshot},async(crops,documentId,sourceHash)=>{
   ...
   await new Promise((resolve,reject)=>{const child=cp.spawn('wsl',['-d',config.wslDistribution||'Ubuntu','--','env','HF_HUB_OFFLINE=1',config.python,toWSL(path.join(__dirname,'dataset/recognize.py')),toWSL(dir)],{windowsHide:true,stdio:['ignore','ignore','pipe']});let error='';child.stderr.on('data',b=>{error=(error+b.toString()).slice(-8000);});const timeout=setTimeout(()=>{child.kill();reject(Error('Recognizer exceeded worker budget'));},600000);child.on('error',reject);child.on('exit',code=>{clearTimeout(timeout);if(code===0)resolve();else{fs.writeFileSync(path.join(dir,'worker-error.txt'),error);reject(Error('Model worker failed; inspect private worker log'));}});});
