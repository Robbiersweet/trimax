/* eslint-disable @typescript-eslint/no-require-imports -- Offline report generation. */
const fs=require('node:fs');
const r=JSON.parse(fs.readFileSync(process.argv[2],'utf8'));
const lines=['# Phase 4 — evidence fusion and confidence contract','',
...
'## 24 real rows','',
`- Raw SVTRv2: ${r.rawSvtr.exact}/24 exact; ${(r.rawSvtr.characterAccuracy*100).toFixed(2)}% character accuracy.`,
`- Fusion top-1: ${r.fusion.exact}/24 (${(r.fusion.exact/24*100).toFixed(2)}%); ${(r.fusion.characterAccuracy*100).toFixed(2)}% character accuracy.`,
`- Candidate-set recall: ${r.fusion.candidateRecall}/24 (${(r.fusion.candidateRecall/24*100).toFixed(2)}%).`,
`- Ambiguous rows requiring review: ${r.fusion.ambiguous}; confidently selected: ${24-r.fusion.ambiguous}.`,
`- Wrongly confident rows: ${r.fusion.wronglyConfident}.`,
`- Invented token characters/digits: ${r.fusion.invented}.`,
'',
...
'|---|---|---|---|---|---|---|---|---|'];
for(const row of r.rows.filter(x=>x.rowId.startsWith('B-'))){
 const e=row.evidence;lines.push(`| ${row.expected} | ${e.observations.map(o=>o.rawText).join(' | ')} | ${e.topCandidate} | ${e.candidates.filter(c=>c.value!==e.topCandidate).map(c=>c.value).join(', ')||'none'} | ${e.ambiguityClass} | ${e.confidence.level} | ${row.expectedLocation} |`);
}
lines.push('','### Aligned disagreements','');
for(const row of r.rows.filter(x=>x.rowId.startsWith('B-'))){const p=row.evidence.positions.filter(p=>p.disputed).map(p=>`${p.anchorIndex===null?'insertion before '+(p.insertionBefore+1):'position '+(p.anchorIndex+1)}: ${p.alternatives.map(a=>a.character??'gap').join(' / ')}`).join('; ');lines.push(`- ${row.expected}: ${p}.`);}
lines.push('',
...
'## Latency','',
`Fusion only, unrelated synthetic observation set: median ${r.latency.cropMedianMs.toFixed(4)} ms per row; ${r.latency.fiveRowsMedianMs.toFixed(4)} ms per five rows; five-row p95 ${r.latency.p95FiveRowsMs.toFixed(4)} ms.`,
