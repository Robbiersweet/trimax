...
test('zero residual remains inspectable without bypassing positive collection rules',()=>{const d=doc(),i=input();i.authoritativeTotal.cents=10000;d.residualEvidence=deriveResidualAmount(i);d.header.total.amount=100;const s=snapshot();s.invoices[0].invoiceAmount=0;const r=resolveOfflineDocument(d,s);assert.equal(r.document.residualEvidence.evidence.derivedAmount,0);assert.equal(r.automaticInvoiceIds.length,0);assert.equal(r.document.rows[0].amounts.length,0);});
