...
new Function('require','module','exports','setTimeout',js)(s=>s.startsWith('.')?require(path.resolve(path.dirname(file),s)):require(s),moduleCopy,moduleCopy.exports,(fn,ms)=>{assert(ms>0&&ms<=5000);return setTimeout(fn,5);});
...
const large=await sharp({create:{width:2000,height:1000,channels:3,background:'white'}}).png().toBuffer();
let step=0;const plan=[.3,'timeout',1,.3,1,.3,1];
const tiered=await probeLegacyDirection(large,async()=>({setParameters:async()=>{},terminate:async()=>{},recognize:()=>{const action=plan[step++];return action==='timeout'?new Promise(()=>{}):Promise.resolve({data:data(action)});}}));
assert.equal(tiered.observations[0].status,'timed-out');assert.equal(tiered.observations[0].previews[0].status,'completed');assert(tiered.observations[0].previews[0].rawText.includes('invoice'));assert.equal(tiered.observations[0].previews[1].status,'timed-out');assert.equal(tiered.observations[1].previews.length,1);assert(tiered.observations.every(o=>o.previews.length<=2));console.log('PASS shared preview deadline, completed small evidence survives fine timeout, readable candidate skips fine preview');
