...
(async()=>{
 ...
 const output=await runShadowPipeline(image,{attemptId:prior.ledger.documentId,captureSessionId:prior.captureSessionId,sourceImageHash:fixture.sha256,build:'private-mature-money-gate',snapshot},async(crops,documentId,sourceHash,moneyCrops,organizationCrops)=>{
  ...
  for(const [i,crop] of organizationCrops.entries()){const file='organization-'+i+'.png';fs.writeFileSync(path.join(batchDir,file),crop.bytes);inputs.push({id:crop.rowId,documentId,file,sha256:crop.sha256,field:'organization'});}
  ...
  return{organizationObservations:data.observations.filter(o=>o.field==='organization').map(o=>{const c=organizationCrops.find(c=>c.rowId===o.id);return{id:'organization:'+o.recognizer+':'+o.id,documentId,sourceImageHash:sourceHash,cropHash:o.sha256,sourceRegion:o.id,geometry:c.bounds,regionType:c.regionType,recognizer:aliases[o.recognizer],rawText:o.raw,confidence:null,durationMs:o.ms};}),observations,versions:data.versions,modelTimings:data.models,moneyObservations:data.observations.filter(o=>['row_amount','total'].includes(o.field)).map(o=>({id:'money:'+o.recognizer+':'+o.field+':'+o.id,rowId:o.id,field:o.field,documentId,sourceHash,cropHash:o.sha256,recognizer:aliases[o.recognizer],raw:o.raw,confidence:null,confidenceCalibrated:false,durationMs:o.ms}))};
 });
 ...
 for(const key of ['checkDate','checkNumber'])assert.deepEqual(output.document.header[key],prior.document.header[key]);
 assert.equal(output.paymentCanApply,false);assert.equal(output.organizationIdentity.authority,'authoritative');assert.equal(output.model.reviewRequired,false);assert.equal(output.pageOnlyIdentity.value,null);assert.equal(output.ledger.entries.filter(e=>e.field==='organization-identity').length,15);assert.equal(output.resolver.status,'automatic');assert.equal(output.resolver.automaticInvoiceIds.length,5);assert.equal(new Set(output.resolver.automaticInvoiceIds).size,5);assert.deepEqual(new Set(output.resolver.automaticInvoiceIds),new Set(JSON.parse(fs.readFileSync(process.argv[4])).find(p=>p.payload.result).payload.result.resolver.candidateAudit.map(r=>r.provisionalInvoiceId)));
 ...
 fs.writeFileSync(path.join(root,'organization-integrated-result.json'),JSON.stringify(output,null,2));
