...
begin
 ...
 perform public.trimax_save_ocr_attempt(b,original,original,null,2,summary,'{"complete":"original"}');
 ...
 perform public.trimax_save_ocr_attempt(b,retry,original,original,2,summary||jsonb_build_object('attemptId',retry,'result','review'),'{"complete":"retry"}');
 ...
 perform public.trimax_save_ocr_attempt(b,success_id,success_id,null,2,summary||jsonb_build_object('attemptId',success_id,'result','success'),'{"must":"discard"}');
...
select 'PASS: terminal immutability, retry linkage, success summary only, pin retention, expiry cleanup, summary preservation, auth gate' as regression;
rollback;
