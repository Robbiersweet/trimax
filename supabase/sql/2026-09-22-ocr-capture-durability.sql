-- Separate image/handoff commits. No payment grants or timeout changes.
...
begin
 ...
 -- Commit durable capture and retry metadata ONLY. Enqueue is a separate request.
 update public.ocr_attempt_diagnostics set payload=payload||jsonb_build_object('captureHandoff',jsonb_build_object(
 'snapshot',case when jsonb_typeof(p_snapshot)='object' and octet_length(p_snapshot::text)<=4000000 then p_snapshot else null end,
 'captureTimings',p_capture,'shadowId',coalesce(p_shadow,gen_random_uuid()))) where attempt_id=p_attempt;
 update public.ocr_attempts set summary=summary||jsonb_build_object('captureState','image_stored','shadowHandoffState',case when p_snapshot is null then 'disabled' else 'handoff_pending' end) where id=p_attempt;
...
begin
 select * into a from public.ocr_attempts where id=p_legacy for update;
 if auth.uid() is null or a.id is null or a.created_by<>auth.uid() or not public.trimax_is_business_admin(a.business_id) then raise exception 'Owner/admin attempt required'; end if;
 ...
 image_reference:=coalesce((a.summary->>'canonicalReference')::uuid,p_legacy);
 if a.summary->>'sourceImageHash' is distinct from p_hash or not exists(select 1 from public.ocr_attempt_optical o join public.ocr_attempts original on original.id=o.attempt_id where o.attempt_id=image_reference and original.business_id=a.business_id and (original.pinned or original.diagnostics_expires_at>now())) then raise exception 'Canonical capture unavailable'; end if;
 else
 ...
 if encode(extensions.digest(decode(bytes,'base64'),'sha256'),'hex') is distinct from p_hash then raise exception 'Canonical capture hash mismatch'; end if;
 select o.attempt_id into image_reference from public.ocr_attempt_optical o where o.attempt_id=coalesce((a.summary->>'canonicalReference')::uuid,p_legacy) and exists(select 1 from jsonb_array_elements(o.evidence->'images') i where i->>'base64'=bytes);
...
-- Serializes retries, including a committed enqueue whose response was lost.
create or replace function public.trimax_resume_ocr_handoff(p_attempt uuid)
returns jsonb language plpgsql security definer set search_path=public as $$
declare a public.ocr_attempts; h jsonb; accepted boolean;
begin
 select * into a from public.ocr_attempts where id=p_attempt for update;
 if auth.uid() is null or a.id is null or a.created_by<>auth.uid() or not public.trimax_is_business_admin(a.business_id) then raise exception 'Owner/admin attempt required'; end if;
 if a.summary->>'canonicalReference' is null then raise exception 'Canonical capture unavailable'; end if;
 if exists(select 1 from public.ocr_shadow_jobs where legacy_attempt_id=p_attempt) then
 accepted:=true;
 else
 select payload->'captureHandoff' into h from public.ocr_attempt_diagnostics where attempt_id=p_attempt;
 if jsonb_typeof(h->'snapshot') is distinct from 'object' then return jsonb_build_object('queued',false,'state','disabled'); end if;
 accepted:=public.trimax_enqueue_ocr_shadow(p_attempt,(h->>'shadowId')::uuid,p_attempt,a.summary->>'sourceImageHash',null,h->'snapshot',h->'captureTimings',null);
 end if;
 update public.ocr_attempts set summary=summary||jsonb_build_object('shadowHandoffState',case when accepted then 'queued' else 'handoff_pending' end) where id=p_attempt;
 return jsonb_build_object('queued',accepted,'state',case when accepted then 'queued' else 'handoff_pending' end);
end $$;
revoke all on function public.trimax_resume_ocr_handoff(uuid) from public,anon;
grant execute on function public.trimax_resume_ocr_handoff(uuid) to authenticated;
...
begin
 ...
 if new.summary->>'captureState' is null or (old.summary->>'canonicalReference' is not null and new.summary->>'captureState' in ('capture_local','uploading','transport_failed')) then new.summary:=new.summary||jsonb_strip_nulls(jsonb_build_object('captureState',old.summary->'captureState')); end if;
 if new.summary->>'shadowHandoffState' is null or (old.summary->>'shadowHandoffState'='queued' and new.summary->>'shadowHandoffState'<>'queued') then new.summary:=new.summary||jsonb_strip_nulls(jsonb_build_object('shadowHandoffState',old.summary->'shadowHandoffState')); end if;
 return new;
end $$;
create or replace function public.trimax_preserve_capture_handoff() returns trigger language plpgsql set search_path=public as $$
begin
 new.payload:=new.payload||jsonb_strip_nulls(jsonb_build_object('canonicalCapture',old.payload->'canonicalCapture','captureHandoff',old.payload->'captureHandoff'));
 return new;
end $$;
drop trigger if exists preserve_capture_handoff on public.ocr_attempt_diagnostics;
create trigger preserve_capture_handoff before update on public.ocr_attempt_diagnostics for each row execute function public.trimax_preserve_capture_handoff();
...
begin
 ...
 diagnostics_expires_at=case when target_result='success' and existing.summary->>'canonicalReference' is null then null else expiry end,
 diagnostic_bytes=case when target_result='success' and existing.summary->>'canonicalReference' is null then 0 when p_payload is null then diagnostic_bytes else octet_length(p_payload::text) end where id=p_id;
 if target_result='success' and existing.summary->>'canonicalReference' is null then delete from public.ocr_attempt_diagnostics where attempt_id=p_id;
...
begin
 ...
 where phase<2 and updated_at<now()-interval '1 hour' and result='processing' and coalesce(summary->>'ocrEngine','legacy')<>'v2-shadow' and coalesce(summary->>'captureState','') not in ('image_stored','processing','transport_failed');
...
select a.id,a.business_id,a.created_at,a.original_id,a.parent_id,a.result,a.summary,a.pinned,a.diagnostics_expires_at,a.diagnostic_bytes,
 ...
 (a.summary->>'captureState' in ('image_stored','transport_failed') or a.summary->>'shadowHandoffState'='handoff_pending' or a.pinned or a.result in ('failed','review','duplicate','apply blocked') or
